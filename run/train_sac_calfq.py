...
class Args:
    ...
    """if toggled, p_relax and p_relax_decay parameters will be annealed"""
    calfq_anneal_frac: float = 0.9
    """the fraction of the total timesteps to anneal p_relax"""
    calfq_selective_buffer: bool = True
    """whether to add only learning-policy transitions to replay buffer"""
...
def main(args: Args):
    ...
    def q_values(obs_np, actions_np):
        torch_obs = torch.tensor(obs_np, dtype=torch.float32, device=device)
        torch_actions = torch.tensor(actions_np, dtype=torch.float32, device=device)
        with torch.no_grad():
            return (
                torch.min(qf1_target(torch_obs, torch_actions), qf2_target(torch_obs, torch_actions))
                .cpu()
                .numpy()
            )
    ...
    best_q_values = np.full((envs.num_envs, 1), fill_value=-np.inf)
    relax_probs = np.full((envs.num_envs, 1), fill_value=args.calfq_p_relax_init)
    p_relax_decay = args.calfq_p_relax_decay
    n_safe_actions = np.zeros((envs.num_envs, 1), dtype=np.float32)
    ...
    for global_step in range(args.total_timesteps):
        ...
        use_policy_mask = np.logical_or(is_q_values_improved, relax_draw)
        current_actions = np.where(use_policy_mask, actions, safe_actions).astype(np.float32)
        relax_probs *= p_relax_decay
        n_safe_actions += (~use_policy_mask).astype(np.float32)

        next_obs, rewards, terminations, truncations, infos = envs.step(current_actions)
        episode_trajectory.append(
            {"obs": obs.copy(), "actions": current_actions.copy(), "reward": np.array(rewards).copy()}
        )
        best_q_values = np.where(is_q_values_improved, current_q_values, best_q_values)
            ...
            if trunc or term:
                ...
                if args.calfq_anneal:
                    ...
                    if rolling_episode_lengths:
                        episode_len = int(np.mean(rolling_episode_lengths))
                    else:
                        episode_len = max(int(np.asarray(infos["final_info"][idx]["episode"]["l"]).item()), 1)
                    reference_init = sum(
                    ...
                    p_relax_decay = args.calfq_p_relax_decay
                metrics_collector.append_metric("calfq/p_relax", float(relax_probs[idx, 0]), step=global_step)
                metrics_collector.append_metric("calfq/p_relax_decay", float(p_relax_decay), step=global_step)
                metrics_collector.log_pending_metrics(synchronous=True)

        if np.any(use_policy_mask) or not args.calfq_selective_buffer:
