...
def main(args: Args):
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                residual_action, logprob, _, value = agent.get_action_and_value(next_obs)
                values[step] = value.flatten()

            actions[step] = residual_action
            logprobs[step] = logprob
            residual_action_np = residual_action.cpu().numpy()
            applied_action_np = (
                residual_action_np + controller.get_action(next_obs_np)
                ...
                episode_trajectories[env_idx].append(
                    {
                        ...
                        "actions": applied_action_np[env_idx].copy(),
