...
class Args:
    ...
    """device to train on"""
    track: bool = False
    """reserved for parity with CleanRL CLI; unused in MLflow setup"""
    ...
    """the rolling average window for metrics"""
...
def main(args: Args):
    ...
    for iteration in range(1, args.num_iterations + 1):
                ...
                episode_trajectories[env_idx].append(
                    {
                        ...
                        "actions": action_np[env_idx].copy(),
        ...
        metrics_collector.append_metric(
            "losses/clipfrac", float(np.mean(clipfracs)) if clipfracs else 0.0, step=global_step
        )
        metrics_collector.append_metric(
